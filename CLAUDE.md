...
Tooltips use `data-tooltip=""` attributes on elements and are rendered by a `<div id="tt" style="position:fixed">` managed entirely by JavaScript. Two global event listeners (`mouseover` and `mouseout`) on the document trigger positioning calculations via `getBoundingClientRect()` in the mount useEffect of `App()`.

**Critical:** NEVER implement tooltips as `[data-tooltip]:hover::after` in CSS. Pseudo-elements with `position:absolute` or `position:fixed` generated by CSS rules trigger layout reflows when they appear/disappear, which can shift ancestor flex containers. This caused a severe bug (v1.21.11) where hovering thumbnails in the library would displace the entire library panel and canvas workspace.

The JS-driven tooltip with `position:fixed` lives completely outside the document layout flow and cannot cause layout shifts.
